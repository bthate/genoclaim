...
def init():
    for name in objs:
        e = Event()
        func = objs.get(name, None)
        todo = Repeater(random.randint(600, 1200), func, e)
        ob.launch(todo.start)
...
def rand(txt):
    return random.choice([x for x in txt.split("\n") if x])

def grond(event):
    k.fleet.announce(grond_txt)

def beleid(event):
    k.fleet.announce(beleid_txt)

def criteria(event):
    k.fleet.announce(criteria_txt)

def geest(event):
    k.fleet.announce(geest_txt)

def machtiging(event):
    k.fleet.announce(machtiging_txt)

objs = ob.Object()
objs.grond = grond
objs.beleid = beleid
objs.criteria = criteria
objs.geest = geest
objs.machtiging = machtiging
